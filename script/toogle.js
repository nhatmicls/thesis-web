...
toogle_1.addEventListener('change', function () {
    if (this.checked) {
        post_data(body_generate("device_1", "led_1", "1"));
    } else {
        post_data(body_generate("device_1", "led_1", "0"));
    }
});

toogle_2.addEventListener('change', function () {
    if (this.checked) {
        post_data(body_generate("device_1", "led_2", "1"));
    } else {
        post_data(body_generate("device_1", "led_2", "0"));
    }
});

toogle_3.addEventListener('change', function () {
    if (this.checked) {
        post_data(body_generate("device_2", "led_1", "1"));
    } else {
        post_data(body_generate("device_2", "led_1", "0"));
    }
});

toogle_4.addEventListener('change', function () {
    if (this.checked) {
        post_data(body_generate("device_2", "led_2", "1"));
    } else {
        post_data(body_generate("device_2", "led_2", "0"));
    }
});